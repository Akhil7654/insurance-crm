...
import {
  getInvestmentClients,
  deleteClient,
  getInvestmentRenewalSummary,
  setInvestmentRenewalDate,
} from '@/lib/api';
...
export default function InvestmentClientsPage() {
  ...
  const loadClients = async () => {
    try {
      ...
      const data = await getInvestmentClients();
      setClients(data);
    } catch {
      setErrorClients('Failed to load investment clients');
      setClients([]);
    } finally {
  ...
  const loadSummary = async () => {
    try {
      ...
      const s = await getInvestmentRenewalSummary(month);
      setSummary(s);
    } catch {
      setSummary(null);
    } finally {
  ...
  const handleDelete = async (e: React.MouseEvent, clientId: number) => {
    e.stopPropagation();
    const ok = confirm('Delete this client? This will remove all notes/conversions etc.');
    if (!ok) return;
  ...
  const handleSetRenewal = async (e: React.MouseEvent, clientId: number) => {
    ...
    try {
      ...
      await setInvestmentRenewalDate(clientId, next);
      await loadClients();
      await loadSummary();
  ...
  return (
    <div className="min-h-screen bg-stone-300 p-6">
      <div className="max-w-2xl mx-auto">
        ...
        >
          <h2 className="text-2xl font-bold text-black">Investment Clients</h2>

          <motion.button
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
            onClick={() => router.push('/investment/add')}
            className="bg-amber-600 text-white px-5 py-2.5 rounded-xl shadow-md cursor-pointer"
          >
            + Add Client
          </motion.button>
        </motion.div>
        ...
        <div className="space-y-3 sm:space-y-4">
          {loadingClients ? (
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center text-gray-500 py-10">
              {search.trim() ? 'No matching investment clients found.' : 'No investment clients found.'}
            </motion.div>
          ) : (
            <AnimatePresence>
              {filteredClients.map((client: any, i) => {
                const renewalDate = client.investment_details?.renewal_date;
                ...
                return (
                  <motion.div
                    ...
                    whileHover={{ y: -2 }}
                    onClick={() => router.push(`/investment/client/${client.id}`)}
                  ...
                  >
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                      <div className="min-w-0">
                        ...
                        {client.investment_details?.investment_type && (
                          <p className="text-xs text-gray-400 mt-1">
                            Type: {client.investment_details.investment_type}
                          </p>
                        )}
                      ...
                      >
                        ...
                        {!client.is_converted && (
                          <motion.button
                            ...
                            className="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-4 py-2 rounded-xl text-sm w-full sm:w-auto"
                          >
                            Convert
                          </motion.button>
                        )}
