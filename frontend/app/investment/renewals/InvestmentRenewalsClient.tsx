...
export default function InvestmentRenewalsListPage() {
  ...
  const load = async () => {
    ...
    try {
      setLoadingList(true);
      const data = await getInvestmentRenewals(month, status);
      setItems(data);
    } finally {
  ...
  const onRenew = async (clientId: number) => {
    ...
    try {
      ...
      await renewInvestmentClient(clientId, next);
      await load();
  ...
  const goToClient = (clientId: number) => {
    router.push(`/investment/client/${clientId}`);
  };
    ...
    <div className="min-h-screen bg-stone-300 p-6">
      <div className="max-w-2xl mx-auto bg-gray-900 border border-gray-800 rounded-2xl p-6">
        <h1 className="text-xl font-bold text-white mb-2">
          Investment Renewals — {status.toUpperCase()}
        </h1>
                ...
                <div className="flex justify-between items-start gap-4">
                  ...
                  >
                    ...
                    <p className="text-gray-300 text-xs mt-2">
                      Investment Type: {it.investment_type || '-'}
                    </p>
                    {it.remarks && (
