...
export default function VehicleClientsPage() {
  ...
  const loadClients = async () => {
    const data = await getVehicleClients();
    setClients(data);
  };

  const loadSummary = async () => {
    try {
      const s = await getVehicleRenewalSummary(month);
      setSummary(s);
    } catch {
      setSummary(null);
    }
  ...
  const handleDelete = async (e: React.MouseEvent, clientId: number) => {
    e.stopPropagation();
    const ok = confirm('Delete this client? This will remove all notes/quotes/docs etc.');
    if (!ok) return;
  ...
  // ✅ Set renewal date from card (only if missing)
  const handleSetRenewal = async (e: React.MouseEvent, clientId: number) => {
    ...
    try {
      ...
      await setVehicleRenewalDate(clientId, next);
      await loadClients();
      await loadSummary();
  ...
  return (
    <div className="min-h-screen bg-stone-300 p-6">
      <div className="max-w-2xl mx-auto">
        ...
        >
          <h2 className="text-2xl font-bold text-black">Vehicle Insurance Clients</h2>

          <motion.button
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
            onClick={() => router.push('/vehicle/add')}
            className="bg-blue-600 text-white px-5 py-2.5 rounded-xl shadow-md cursor-pointer"
          >
            + Add Client
          </motion.button>
        </motion.div>
            ...
            {clients.map((client: any, i) => {
              ...
              return (
                <motion.div
                  ...
                  whileHover={{ y: -2 }}
                  onClick={() => router.push(`/vehicle/client/${client.id}`)}
                ...
                >
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    ...
                    >
                      {/* ✅ Set Renewal button only if missing */}
                      {needsRenewalDate && (
                        <motion.button
                          whileTap={{ scale: 0.96 }}
